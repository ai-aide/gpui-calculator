use gpui::{
    div, prelude::*, px, size, App, Application, Bounds, Context, TextOverflow, Window, WindowAppearance, WindowBounds, WindowOptions
};

struct HelloWord {}

impl Render for HelloWord {
...
fn main() {
    Application::new().run(|cx: &mut App| {
       let bounds = Bounds::centered(None, size(px(800.0), px(600.)), cx);
       cx.open_window(
           WindowOptions {
               window_bounds: Some(WindowBounds::Windowed(bounds)),
               ..Default::default()
           },
           |_, cx| cx.new(|_| HelloWord {}),
       )
    .unwrap();
    });
}
